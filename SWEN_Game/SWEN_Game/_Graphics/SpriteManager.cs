    ...
    public class SpriteManager
    {
        ...
        public Texture2D GetTilesetTextureFromRenderer(LDtkLevel level, string tilesetPath)
        {
            if (Globals.Content == null)
            {
                ...
                string assetName = Path.Join(directory, tilesetPath);
                return Texture2D.FromFile(Globals.Graphics.GraphicsDevice, assetName);
            }
            else
            {
                string file = Path.ChangeExtension(tilesetPath, null);
                string directory = Path.GetDirectoryName(level.WorldFilePath) !;
        ...
        /// </returns>
        public virtual Dictionary<string, Dictionary<int, List<Vector2>>> GetTileGroups()
        {
            return tileGroups;
        }
        ...
        /// </returns>
        public virtual Dictionary<string, List<int>> GetTileMappings()
        {
            return tileMappings;
        }
        ...
        public virtual float GetDepth(Vector2 position, float spriteHeight, LayerInstance layer)
        {
        ...
        public virtual float GetDepth(Vector2 position, float spriteHeight)
        {
        ...
        public virtual float GetDepth(float depth, LayerInstance layer)
        {
            float layerOffset = 0;
            switch (layer._Identifier)
            {
